...
type GestureSyntaxKey = 'swipe' | `gesture ${(typeof GESTURE_INPUT_KINDS)[number]}`;
...
export function normalizePublicGesture(input: GesturePayload): NormalizedPublicGesture {
  switch (input.kind) {
    ...
    case 'drag':
      throw new AppError(
        'INVALID_ARGS',
        'gesture drag targets must be resolved by the interaction runtime',
      );
  }
}
