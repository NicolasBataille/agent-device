...
Reporter modules can export a reporter object, `reporter`, `createReporter`, or a default factory. Factories receive load context. Reporter hooks receive an IO context with `stdout`, `stderr`, `mkdir`, and `writeFile` helpers:
...
export default function createReporter(loadContext) {
  return {
    ...
    onProgress(event, context) {
      if (event.type === 'replay-test' && event.status === 'progress') {
        context.stderr.write(`running ${event.file} ${event.stepIndex}/${event.stepTotal}\n`);
      }
    },
...
export default {
  ...
  onProgress(event, context) {
    ...
    if (!['pass', 'fail', 'skip'].includes(event.status)) return;

    const icon =
      event.status === 'pass' ? '✅' : event.status === 'fail' ? '❌' : '⏭️';
    const title = event.title?.trim() || event.file;
    const duration =
...
const createReporter: ReplayTestReporterFactory = () => ({
  name: 'typed-reporter',
  onProgress(event, context) {
    // Render live updates from replay-test and replay-test-suite events.
    context.stderr.write(`${event.type}\n`);
  },
  onSuiteEnd(suite) {
...
`onProgress` receives streamed `replay-test-suite`, `replay-test`, and command progress events while the daemon request is running. `onSuiteEnd` receives the final suite result. `getExitCode` can override whether the finished suite exits successfully; when no reporter supplies one, failed tests exit with `1`.
