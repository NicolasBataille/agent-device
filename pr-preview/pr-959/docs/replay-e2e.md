...
TypeScript reporters can infer the reporter shape from the exported default reporter factory:

```ts
import {
  createDefaultReplayTestReporter,
  createReplayTestProgressRenderer,
} from 'agent-device';

type ReplayTestReporter = ReturnType<typeof createDefaultReplayTestReporter>;
