...
- On hosted WebDriver sessions (BrowserStack, AWS Device Farm), `fill` waits until the field it tapped actually holds text entry focus before sending its keys, and reports what it observed as `textEntryReadiness`: `focused-element` (the tap moved focus, or landed inside the element already focused — the strongest signal, and the only one that identifies _which_ field took focus) or `keyboard-shown` (the driver has no active-element route, but the keyboard rose from hidden after the tap). Both mean focus was witnessed before any key was sent; `fill` never types without evidence.
- If focus cannot be witnessed, `fill` **fails** and sends no keys, rather than reporting a filled field that never received the text.
  - `text_entry_focus_not_observed` — the tap focused nothing, or the keyboard was already up on a driver that cannot name the focused field (there, unwitnessed keys would land in the field the previous fill focused). This almost always means the tap missed: re-check the target with `snapshot -i` and fill the element it reports instead of retrying the same coordinates.
  - `text_entry_focus_unobservable` — the driver reports neither the focused element nor keyboard state, so nothing could establish focus. Use `press <target>` followed by `type <text>` to enter the text anyway, accepting that nothing witnesses which field receives the keys.
