...
Use `client.sessions.artifacts({ provider, providerSessionId })` with `closed.provider?.providerSessionId` to fetch hosted video/log URLs after close when the provider supports them. See the [BrowserStack](/agent-device/pr-preview/pr-1695/docs/browserstack.md), [AWS Device Farm](/agent-device/pr-preview/pr-1695/docs/aws-device-farm.md), and [Limrun](/agent-device/pr-preview/pr-1695/docs/limrun.md) guides for provider setup and workflows.
