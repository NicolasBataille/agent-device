...
Use `client.sessions.artifacts({ provider, providerSessionId })` with `closed.provider?.providerSessionId` to fetch hosted video/log URLs after close when the provider supports them. See [Device Clouds & Farms](/agent-device/pr-preview/pr-1574/docs/device-clouds.md) for Limrun, BrowserStack, AWS Device Farm, CLI, JavaScript, and MCP flows.
