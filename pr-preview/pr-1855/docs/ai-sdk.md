...
`agent-device mcp` starts the same [MCP server](/agent-device/pr-preview/pr-1855/docs/agent-setup.md#mcp-server) documented for Claude Code, Cursor, and other MCP clients — every installed command, as a structured tool, over the same execution path as the CLI. [`@ai-sdk/mcp`](https://ai-sdk.dev/docs/ai-sdk-core/mcp-tools)'s `createMCPClient()` connects to it and converts its tools into AI SDK tools directly, so there is no `agent-device` import at all:
...
See [Node.js API](/agent-device/pr-preview/pr-1855/docs/client-api.md) for the complete client surface and runnable, typechecked `agent-device` examples.
