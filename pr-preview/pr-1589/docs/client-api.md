...
Use `client.sessions.artifacts({ provider, providerSessionId })` with `closed.provider?.providerSessionId` to fetch hosted video/log URLs after close when the provider supports them. See [Device Clouds & Farms](/agent-device/pr-preview/pr-1589/docs/device-clouds.md) for Limrun, BrowserStack, AWS Device Farm, CLI, JavaScript, and MCP flows.
...
Selector parsing and matching are internal to the CLI/daemon runtime. Consumer code should use the
typed client methods or CLI commands such as `find`, `click`, `fill`, and `wait`; the former
`agent-device/selectors` AST-exporting subpath is no longer part of the public package.
