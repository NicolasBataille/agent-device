...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-1536/docs/configuration.md).
...
- Use [Remote Proxy](/agent-device/pr-preview/pr-1536/docs/remote-proxy.md) when you need to run `agent-device proxy` on a Mac with simulator/device access and drive it from another machine through cloudflared, ngrok, or another HTTP tunnel.
- Use [Device Clouds & Farms](/agent-device/pr-preview/pr-1536/docs/device-clouds.md) when agents need BrowserStack or AWS Device Farm sessions from CI without interactive login.
...
[Migrating Gestures](/agent-device/pr-preview/pr-1536/docs/migrating-gestures.md) if you have scripts or recordings that still pass one.
...
- `replay -u`/`--update` no longer rewrites the script (retired — see [Replay & E2E](/agent-device/pr-preview/pr-1536/docs/replay-e2e.md)); it is a no-op kept for compatibility, since every divergence already carries the same ranked suggestions.
- `--save-script` records a replay script on `close`; optional path is a file path and parent directories are created.

See [Replay & E2E](/agent-device/pr-preview/pr-1536/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-1536/docs/batching.md) for payload format, response shape, and usage guidelines.
