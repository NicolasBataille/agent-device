...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-844/docs/configuration.md).
...
- Use [Remote Proxy](/agent-device/pr-preview/pr-844/docs/remote-proxy.md) when you need to run `agent-device proxy` on a Mac with simulator/device access and drive it from another machine through cloudflared, ngrok, or another HTTP tunnel.
...
See [Replay & E2E](/agent-device/pr-preview/pr-844/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-844/docs/batching.md) for payload format, response shape, and usage guidelines.
