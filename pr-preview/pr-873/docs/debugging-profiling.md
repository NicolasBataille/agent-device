...
- For React Native JavaScript heap leaks, use `agent-device cdp` against the Metro CDP target instead of native/process memory samples. Start with `memory usage sample --gc`, then confirm retained objects with `memory snapshot diff`, `memory snapshot leak-triplet`, and `memory snapshot retainers`.
