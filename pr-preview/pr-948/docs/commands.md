...
For persistent defaults and project-scoped CLI settings, see [Configuration](/agent-device/pr-preview/pr-948/docs/configuration.md).
...
See [Replay & E2E](/agent-device/pr-preview/pr-948/docs/replay-e2e.md) for recording, Maestro compatibility, and CI workflow details.
...
See [Batching](/agent-device/pr-preview/pr-948/docs/batching.md) for payload format, response shape, and usage guidelines.
...
- `disconnect --json` and `close --json` include provider release data when the runtime returns final cloud artifacts after session teardown. Some providers only finalize video/log URLs after the remote session is stopped, so retry `artifacts --provider ... --provider-session ...` if the first response is `pending`.
