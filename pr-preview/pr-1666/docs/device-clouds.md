...
- On hosted WebDriver sessions (BrowserStack, AWS Device Farm), `fill` waits for the tapped field to take text entry focus before sending its keys, and reports what it observed as `textEntryReadiness`: `keyboard-shown` (the keyboard came up in response to the tap), `settled-keyboard-up` (the keyboard was already up — back-to-back fills into one form — so it could not witness the new field taking focus and the fill waited out its full readiness budget), `settled-unknown` (the driver reports no keyboard state), or `not-observed` (no keyboard ever appeared — the keys were still sent, but nothing corroborates that a field received them). A `not-observed` fill on a field you expected to focus usually means the tap missed: re-check the target with `snapshot -i` rather than retrying the same coordinates.
