...
- Failure diagnostics render resolved variables used as targets and `runFlow` paths, while `inputText`
  payloads remain hidden. Do not place secrets in selectors, links, filenames, or other diagnostic
  identifiers that are expected to appear in failure output.
