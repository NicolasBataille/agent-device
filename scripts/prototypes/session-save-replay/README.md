...
interaction happens and cannot be reconstructed by an end-only save. `session save-replay` publishes
`open` plus all recorded actions, never adds `close`, and disarms close-time publication. Replaying the
artifact therefore starts from scratch and leaves the session active at the destination. An existing
path is refused unless `--force` is present.

## Run

```sh
pnpm prototype:session-save-replay
```
...
Intermediate fragments, arbitrary start/stop ranges, credentials policy, `include`, and composed-flow
healing are intentionally out of scope.
