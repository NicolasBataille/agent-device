...
function normalizeLease(data: Record<string, unknown>): Lease {
  ...
  return {
    ...
    leaseProvider: readOptionalString(lease, 'leaseProvider'),
    provider: readOptionalString(lease, 'provider') as Lease['provider'],
    clientId: readOptionalString(lease, 'clientId'),
    deviceKey: readOptionalString(lease, 'deviceKey'),
