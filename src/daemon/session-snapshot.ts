 ...
 * stored — nothing was issued, so the frame is left exactly as the action's
 * own side-effect seam expired it.
 *
 * Every route that can attach a settle observation goes through here (touch
 * commands and the generic scroll/back route, #1638), so the issuance rule has
 * one implementation rather than one per handler.
 */
export function issueSettleObservationRefs(
  session: SessionState,
