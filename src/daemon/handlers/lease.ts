...
export async function handleLeaseCommands(args: LeaseHandlerArgs): Promise<DaemonResponse | null> {
  ...
  switch (req.command) {
    ...
    case 'lease_release': {
      const lease = findLeaseForRelease(leaseRegistry, leaseScope);
      const providerData = lease ? await leaseLifecycleProvider?.release?.(lease) : undefined;
      const result = leaseRegistry.releaseLease(leaseScopeToReleaseRequest(leaseScope));
      return {
        ok: true,
        data: { ...result, ...(providerData ? { provider: providerData } : {}) },
      };
    }
    default:
      return null;
...
function findLeaseForRelease(
  leaseRegistry: LeaseRegistry,
  leaseScope: ReturnType<typeof resolveLeaseScope>,
): DeviceLease | undefined {
  try {
    return leaseRegistry.heartbeatLease(leaseScopeToHeartbeatRequest(leaseScope));
  } catch {
    return undefined;
  }
}
